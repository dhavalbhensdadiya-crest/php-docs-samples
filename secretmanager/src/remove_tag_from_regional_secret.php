...
// [START secretmanager_remove_tag_from_regional_secret]
...
/**
 * Remove a tag binding from a regional secret.
 *
 ...
 */
function remove_tag_from_regional_secret(string $projectId, string $locationId, string $secretId, string $tagValue): void
{
    ...
    foreach ($pagedResponse->iterateAllElements() as $binding) {
        if ($binding->getTagValue() === $tagValue) {
            ...
            if ($operation->operationSucceeded()) {
                printf('Deleted tag binding %s' . PHP_EOL, $binding->getName());
            } else {
                $error = $operation->getError();
                printf('Error deleting tag binding: %s' . PHP_EOL, $error->getMessage());
            }
    ...
    printf('No tag binding found for tag value %s on secret %s' . PHP_EOL, $tagValue, $parent);
}
// [END secretmanager_remove_tag_from_regional_secret]
